...
import type {Camera, ImageRecord, ModelStats, Point3D, Reconstruction, Vec3} from "./types";
...
export class ReconstructionViewer {
  ...
  get modelStats(): ModelStats | null {
    if (!this.reconstruction) return null;
    const errors = this.reconstruction.points3D.errors;
    let errorSum = 0;
    for (let i = 0; i < errors.length; ++i) errorSum += errors[i]!;
    return {
      registeredImages: this.reconstruction.images.size,
      cameras: this.reconstruction.cameras.size,
      points: errors.length,
      meanReprojectionError: errors.length > 0 ? errorSum / errors.length : Number.NaN,
    };
  }
